...
Y_e2<-list()
for(i in 1:nT){
  Y_e2[[i]]<-Ylist[[i]]-W%*%diag(rhohat)%*%(Ylist[[i]])-Zlist[[i]]%*%betahat
}
errmatrix<-matrix(rep(0,N*nT),N,nT)
Ymatrix<-matrix(rep(0,N*nT),N,nT)
for (i in 1:nT){
  errmatrix[,i]<-Y_e2[[i]]
  Ymatrix[,i]<-Ylist[[i]]
}

acfe<-matrix(rep(0,N*5),N,5)
acfy<-matrix(rep(0,N*5),N,5)
for (i in 1:N){
  aay<-acf(Ymatrix[i,],lag.max=5,ci=1-0.05/55)
  acfy[i,]<-aay$acf[-1]
  aae<-acf(errmatrix[i,],lag.max=5,ci=1-0.05/55)
  acfe[i,]<-aae$acf[-1]
}

acfymatrix<-round(acfy,4)
acfematrix<-round(acfe,4)
rownames(acfymatrix)<-c("Austria","Belgium","Finland","France","Germany","Greece","Ireland","Italy","Netherlands","Portugal","Spain")
colnames(acfymatrix)<-c("Lag-1","Lag-2","Lag-3","Lag-4","Lag-5")
rownames(acfematrix)<-c("Austria","Belgium","Finland","France","Germany","Greece","Ireland","Italy","Netherlands","Portugal","Spain")
colnames(acfematrix)<-c("Lag-1","Lag-2","Lag-3","Lag-4","Lag-5")

xtable(acfymatrix,digits = 4)
xtable(acfematrix,digits = 4)